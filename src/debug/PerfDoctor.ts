 ...
 *
 * 📖 **完整文档：`docs/03-runtime/perf-doctor.md`** —— 用法、规则表、五条铁律、
 *    历史根因与已踩死路清单全在那里。改这个文件前先读它。
 *
 ...
 *   · 持续卡顿（8-31 最终定案）：`LegionPhalanxDrawer.unitSpriteCache` **无上限、占 12961MB**。
 *     🔴 之前查不出来，是因为**已解码位图不在 JS 堆里** —— heap 只报 702MB 看着很健康。
 *     补上缓存体检后一测即现。**判断图片内存只看 caches 段，别看 heap。**
    ...
    public report(context: Record<string, unknown> = {}): Record<string, unknown> {
        ...
        return {
            ...
            aiGuide: {
                ...
                doc: 'docs/03-runtime/perf-doctor.md —— 完整用法、规则表、历史根因清单都在这里，先读它。',
                readFirst: [
                    '先看 findings；它已按规则给出定位与修法。hotspots/caches 是原始证据。',
                    'findings 为空不等于不卡：可能是探针没登记。检查 caches 是否覆盖了所有图片/位图缓存。',
                ],
                deadEnds: [
                    '🔴🔴 **最重要的一条**：别用 performance.memory / heap 判断「内存有没有问题」。',
                    '   **已解码的图片位图不在 JS 堆里**，heap 完全看不到它。2026-08-31 实测：',
                    '   heap 只报 702MB（上限 4096，看着很健康），而 unitSpriteCache 实占 **12961MB**。',
                    '   → 判断图片内存**只看本报告 caches 段的 MB**，那是逐张 w×h×4 实算的；heap 段只反映 JS 对象。',
                ...
                measuringTips: [
                    ...
                    '⚠️ 新建缓存时预算别卡在稳态工作集上；但**反过来不成立**：',
                    '   看到某缓存「占用顶在预算上」，**不能**据此断定它在抖动、抬预算就能变快。',
                    '   判抖动的唯一依据是本报告 caches[].churn.reAdds（被淘汰后又加回来的条数）。',
                    '   2026-08-31 血训：我拿「顶死在 599.6/600MB」当抖动证据，把四个缓存预算翻倍；',
                    '   事后加计数器实测 reAdds 全是 0，从来没抖过 —— 抬预算对帧率毫无帮助，',
                    '   只把解码位图从 1.7GB 堆到 3.7GB。当天真正治好卡顿的是**开机改按需加载**。',
                    '⚠️ 加淘汰之前先确认「淘汰掉的东西能不能自动回来」：没有按需重载就淘汰 = 内容永久消失。',
                    '   2026-08-31 兵种贴图是先补了 ensureUnitTypeLoading 按需加载，才敢开淘汰的。',
                    '⚠️ **别为了省事在开机把资源全预载一遍**。2026-08-31 实测：开机全量解码 322 个兵种',
                    '   （每个都要抠绿），其中 144 个装完就被淘汰、全程一次没用过（evicts:144/reAdds:0）。',
                    '   改成只预载兜底两种、其余按需：开机长任务 600 次/160 秒 → 39 次/7.1 秒，最长 4804ms → 1066ms。',
