...
function startFieldBattleBetween(
    ...
    // 岳飞北伐：每战必胜·攻守皆胜（仅北伐脚本激活期间生效，防主游戏岳家军野战被误判——2026-07-19 主人定）
    const yuefeiExpeditionOn = !!(window as any).__yuefeiExpeditionActive;
    const hasYuefei = (l: unknown) => (l as any).generalId === 'yanchuan_d_yuefei';
    const yuefeiOnAttack = yuefeiExpeditionOn && attLegions.some(hasYuefei);
    const yuefeiOnDefense = yuefeiExpeditionOn && defLegions.some(hasYuefei);
    const preset = yuefeiOnAttack
