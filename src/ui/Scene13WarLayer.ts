...
// 兵种属性表 WAR_TYPES 与 WarType 已移至 src/data/WarTypes.ts（纯数据叶子模块，编辑器也要读）
...
const SIEGE_TECH_BY_CULTURE: Record<RegionType, Record<string, boolean>> = {
    ...
    SLAVIC_FEUDAL: { battering_ram: true, capped_ram: true, siege_ram: true, scorpion: true, heavy_scorpion: true, mangonel: true, onager: true, siege_onager: true },
    SLAVIC_CASTLE: { battering_ram: true, capped_ram: true, siege_ram: true, scorpion: true, heavy_scorpion: true, mangonel: true, onager: true, siege_onager: true },
    SLAVIC_IMPERIAL: { battering_ram: true, capped_ram: true, siege_ram: true, scorpion: true, heavy_scorpion: true, mangonel: true, onager: true, siege_onager: true },
    ...
    GERMANIC_FEUDAL: { battering_ram: true, capped_ram: true, scorpion: true, heavy_scorpion: true, mangonel: true, onager: true },
    GERMANIC_IMPERIAL: { battering_ram: true, capped_ram: true, scorpion: true, heavy_scorpion: true, mangonel: true, onager: true },
    GERMANIC_CASTLE: { battering_ram: true, capped_ram: true, scorpion: true, heavy_scorpion: true, mangonel: true, onager: true },
    LATIN: { battering_ram: true, capped_ram: true, siege_ram: true, scorpion: true, heavy_scorpion: true, mangonel: true, onager: true },
    LATIN_CASTLE: { battering_ram: true, capped_ram: true, siege_ram: true, scorpion: true, heavy_scorpion: true, mangonel: true, onager: true },
    LATIN_IMPERIAL: { battering_ram: true, capped_ram: true, siege_ram: true, scorpion: true, heavy_scorpion: true, mangonel: true, onager: true },
    ...
    STEPPE_FEUDAL: { battering_ram: true, capped_ram: true, siege_ram: true, scorpion: true, heavy_scorpion: true, mangonel: true, onager: true },
    ...
    CENTRAL_ASIA_ANTIQUITY: { battering_ram: true, capped_ram: true, siege_ram: true, scorpion: true, heavy_scorpion: true, mangonel: true },
    CENTRAL_ASIA_CASTLE: { battering_ram: true, capped_ram: true, siege_ram: true, scorpion: true, heavy_scorpion: true, mangonel: true },
    ...
    TIBET_CASTLE: { battering_ram: true, capped_ram: true, siege_ram: true, scorpion: true, heavy_scorpion: true, mangonel: true, onager: true, siege_onager: true },
    ...
    WEST_ASIA_ANTIQUITY: { battering_ram: true, capped_ram: true, siege_ram: true, scorpion: true, mangonel: true, onager: true },
    WEST_ASIA_CASTLE: { battering_ram: true, capped_ram: true, siege_ram: true, scorpion: true, mangonel: true, onager: true },
    ...
    AFRICA_ANTIQUITY: { battering_ram: true, capped_ram: true, scorpion: true, mangonel: true, onager: true, siege_onager: true },
    AFRICA_CASTLE: { battering_ram: true, capped_ram: true, scorpion: true, mangonel: true, onager: true, siege_onager: true },
    ...
    SEASIA_CASTLE: { battering_ram: true, capped_ram: true, scorpion: true, heavy_scorpion: true, mangonel: true, onager: true, battle_elephant: true },
    SEASIA_FEUDAL: { battering_ram: true, capped_ram: true, scorpion: true, heavy_scorpion: true, mangonel: true, onager: true, battle_elephant: true },
    ...
    YARLUNG: { battering_ram: true, capped_ram: true, scorpion: true, mangonel: true },
...
interface WarBuilding {
    ...
    linked?: boolean;
    /** 城墙破损档素材目录名数组 [D25, D50, D75]；有值 → 破墙前按 HP 比例渐进切换，破墙后切 D75 残垣；无值（木栅栏）→ 直接消失 */
    destrAssets?: string[];
...
interface ArrowTower {
    ...
    /** 🔴 30 秒随机切成残骸后置位：不再开火（完整/破损保持 false 继续射） */
    down: boolean;
}
...
/**
 * 巡逻航点**切换**后的驻足停留时长（秒）——见 advanceRoute / aimAt。
 * 🔴 [2026-08-22 主人定] 寻敌军团变更目的地后停一下再走：
 *   巡逻航路的目的就是搜索场上残敌，若航点一到就立刻掉头奔下一个点，
 *   整军永远在赶路不停脚；敌方残兵主动追上来打，却怎么都追不上、打不成，
 *   画面就是「残兵追着跑、寻敌方一路走」打不起来。停这一下，残兵才追得上来交上火。
 */
const ROUTE_HOLD_SEC = 1.5;
 ...
 */
/**
 * 🔴🔴 [2026-09-23 主人定，取代下面 9-15 那条]「现在战略，战术都改为10队」「我现在说的才是硬规定」。
 *   10 口 = 编制 9 口（前中后三排，LAYOUT 表仍是 9 格）+ **主将队 1 口**（前排正中再往前，见 COMMANDER_CELL）。
 *   主将队兵种：主将专属英雄素材 > 本军团前排兵种（src/data/generalHeroUnits.ts）。
 *
 * 🔴 [2026-09-15 主人定]「任何时候双方都必须是 9 支军队，永远是 9」（已被 9-23 改为 10）。
...
const DE_DYN_DIRS = ["/SUCAI/GUANYU/","/SUCAI/AMAZONARCHER/","/SUCAI/AMAZONWARRIOR/","/SUCAI/ANTIQUITY_BATTERINGRAM/","/SUCAI/ANTIQUITY_CAPPED_RAM/","/SUCAI/ANTIQUITY_CAVALRY_ARCHER/","/SUCAI/ANTIQUITY_HEAVY_CAVALRY_ARCHER/","/SUCAI/ANTIQUITY_HEAVY_SCORPION/","/SUCAI/ANTIQUITY_LIGHT_CAVALRY/","/SUCAI/ANTIQUITY_MANGONEL/","/SUCAI/ANTIQUITY_ONAGER/","/SUCAI/ANTIQUITY_SCORPION/","/SUCAI/ANTIQUITY_SCOUT_CAVALRY/","/SUCAI/ANTIQUITY_SIEGE_ONAGER/","/SUCAI/ANTIQUITY_SIEGE_RAM/","/SUCAI/ANTIQUITY_SIEGE_TOWER/","/SUCAI/ANTIQUITY_SKIRMISHER/","/SUCAI/ANTIQUITY_SPEARMAN/","/SUCAI/ARAMBAI/","/SUCAI/ARBALEST/","/SUCAI/ARCHER/","/SUCAI/ARMORED_ELEPHANT/","/SUCAI/BACTRIAN_ARCHER/","/SUCAI/BALLISTA_ELEPHANT/","/SUCAI/BATTERINGRAM/","/SUCAI/BATTLEELEPHANT/","/SUCAI/BAYINNAUNG_ELEPHANT/","/SUCAI/BERSERK/","/SUCAI/BLACKWOODARCHER/","/SUCAI/BOLASRIDER/","/SUCAI/BOMBARDCANNON/","/SUCAI/BOWMAN/","/SUCAI/BOYAR/","/SUCAI/CAMELARCHER/","/SUCAI/CAMELRIDER/","/SUCAI/CAMELSCOUT/","/SUCAI/CAMEL_HEAVY/","/SUCAI/CAMEL_RAIDER/","/SUCAI/CAPPEDRAM/","/SUCAI/CATAPHRACT/","/SUCAI/CAVALIER/","/SUCAI/CAV_ARCHER/","/SUCAI/CAV_ARCHER_HEAVY/","/SUCAI/CENTURION/","/SUCAI/CHAKRAMTHROWER/","/SUCAI/CHAMPION/","/SUCAI/CHAMPIRUNNER/","/SUCAI/CHAMPISCOUT/","/SUCAI/CHAMPIWARRIOR/","/SUCAI/CHUKONU/","/SUCAI/COMPANION_CAVALRY/","/SUCAI/COMPOSITE_BOWMAN/","/SUCAI/CONDOTTIERO/","/SUCAI/CONQUISTADOR/","/SUCA
GUARD/","/SUCAI/TEUTONICKNIGHT/","/SUCAI/THRACIAN_PELTAST/","/SUCAI/THROWING_AXEMAN/","/SUCAI/TIGER_RIDER/","/SUCAI/TRACTIONTREBUCHET/","/SUCAI/TWOHANDEDSWORDSMAN/","/SUCAI/URUMISWORDSMAN/","/SUCAI/VANGUARD/","/SUCAI/WARCHARIOT/","/SUCAI/WARDOG/","/SUCAI/WARRIORPRIEST/","/SUCAI/WARWAGON/","/SUCAI/WAR_CHARIOT/","/SUCAI/WAR_ELEPHANT/","/SUCAI/WHITE_FEATHER_GUARD/","/SUCAI/WINGEDHUSSAR/","/SUCAI/WOADRAIDER/","/SUCAI/XIANBEI_RAIDER/","/SUCAI/XOLOTLWARRIOR/"];
...
/**
 * 🔴 [2026-08-26 主人「所有高级建筑都安置上了吗」] 势力级建筑风格集（覆盖 REGION_BUILDING_STYLE）。
 *
 *    盘点发现 4 套**全套风格集整套闲置**（各 43~45 件），其中一条还是史实错误：
 *      · ANDE（安第斯）—— 印加/马普切/穆伊斯卡本该用它，却跟着 AMERICA 区用了
 *        **MESO（中美洲）**。安第斯石构与玛雅/阿兹特克金字塔完全两回事，这是错配。
 *      · PERSIAN —— 波斯本该用它，却跟着 WEST_ASIA 区用了 ORIE（通用中东）。
 *      · EAST（东欧/日耳曼东部）—— 从奇观命名 EAST_WONDER_GOTHS/HUNS/TEUTONS/VIKINGS
 *        可确认它是给哥特/匈人/条顿/维京的，这些势力却在用 WEST/CEAS。
 *      · PURU（南亚）—— 见 FACTION_CASTLE 里的更正说明。
 *    风格集是整套的（兵营/房屋/塔/墙/门都换），所以按**势力**挂，别动 REGION_BUILDING_STYLE。
 */
const FACTION_BUILDING_STYLE: Readonly<Record<string, string>> = {
    // ── 安第斯（ANDE）：与中美洲 MESO 分开 ──
...
const PROJ_ASSET_KEY: Record<string, string> = {
    PROJ_CROSSBOW: 'PROJ_ARROW',   // [2026-09-14] 弩兵：画的是 DE 小箭，只是飞得比弓箭平（见 PROJ_ARC_RATIO）
...
const PROJ_TYPE: Record<string, string> = {
    ...
    antiquity_siege_onager: 'PROJ_MANGONEL',
    ...
    //    ⚠️ 但**不能挂 PROJ_BOLT**：那是弩炮的粗弩矛（52×12、**仅 1 帧**，朝哪射都同一个角度），
    //       而 DE 里弩手与诸葛弩射的就是小箭（见下方注释「箭：弓手/弩手/…/诸葛弩」）。
    //    故新建逻辑弹丸 PROJ_CROSSBOW：**素材仍用 PROJ_ARROW 小箭**（11 帧带方向），
    //    只借独立 key 拿自己的飞行参数（0.09 平直）—— 正是 PROJ_ASSET_KEY 那层间接的用途。
    crossbowman: 'PROJ_CROSSBOW',
    arbalest: 'PROJ_CROSSBOW',
    chukonu: 'PROJ_CROSSBOW',
    elite_chukonu: 'PROJ_CROSSBOW',
    genoese_crossbowman: 'PROJ_CROSSBOW',
    elite_genoese_crossbowman: 'PROJ_CROSSBOW',
    gastraphetes: 'PROJ_CROSSBOW',
    ...
    // 投石机/抛石机（投石弹/巨石，高抛弧线 + 落地碎石泥土冲击）
    mangonel: 'PROJ_MANGONEL',
    onager: 'PROJ_MANGONEL',
    siege_onager: 'PROJ_MANGONEL',
    traction_trebuchet: 'PROJ_ROCK',
    mounted_trebuchet: 'PROJ_ROCK',
...
const PROJ_ARC_RATIO: Record<string, number> = {
    ...
    //    实算峰值（弧高 = min(射程×系数, 100)，峰值 = 4×弧高×0.25）：
    //      弩兵 280 → 25px ｜ 热那亚弩手 320 → 29px ｜ 标枪 150 → 27px ｜ 弓兵 160 → 48px
    PROJ_CROSSBOW: 0.09,       // 弩兵（弩/诸葛弩/热那亚弩/腹弩）：初速最高，弹道最平
    PROJ_SPEAR: 0.18,          // 标枪/投矛：比弓箭直，仍看得出抛投
    PROJ_SPEAR_SMALL: 0.18,    // 小标枪（战车投矛）同档
    // ── [2026-09-12 新增] 平射弹按 DE 的负 arc ──
    // 🔴 [2026-09-14 主人定]「看看战略地图上军团的射箭弧度（哪个是好的）」——**以战略地图那套为准**。
    //    战略地图 ProjectileRenderer 的公式与这里**逐字相同**：arcHeight = min(dist × 0.3, 100)，
    //    visualZ = 4 × arcHeight × p × (1−p)；区别只在系数恒为 **0.3**，且永不取负。
    //    09-12 那次「照 DE dat 实值重填」把弓/弩箭填成 −0.06，而本渲染器 y = groundY − lift − arc，
    //    **负 arc = 弹道沉到地面以下**，实算中点低于地面约 9.6px（士兵才 40px 高）→ 箭一路穿地。
    //    现把这两项**整个删除**，回落到默认 0.3：弓箭 / 弩箭（弩兵与弓兵共用 PROJ_ARROW）/ 标枪
    //    三者与战略地图一模一样。⚠️ DE 的 projectile_arc 与本渲染器的屏幕弧高不是一个语义，别再照搬数值。
...
/**
 * 每方开局数 + 每次补兵批量 = 360（2026-08-18 主人改「9 口 × 36 = 每口 6×6 方正」；2026-09-23 主人改 10 口 → 10 × 36 = 360）。
 * 成批补：开局双方各出 360；之后一方场上 < 150 才再补 360（见 TRIGGER）。
 */
const SIDE_CAP = 360;
/**
 * 补兵触发线 = 180（2026-08-13 主人定「150」；2026-09-23 主人改 SIDE_CAP 360 → 打掉一半 = 180）：一方场上掉到 180 以下才再补一波 360。
 * 成批补制造「兵力波次」：一方打掉一半（360→180）才补，战线来回摆动，尸体沿途铺开。
 */
const TRIGGER = 180;
/**
 ...
 */
const FLEE_SPD = 150;
/** 溃逃总时长（秒）：反向移动 + 渐隐，跑完即消失；4 秒约跑 600px */
const FLEE_DUR = 4.0;
/**
...
interface WarSpawn {
    ...
    playerElite?: boolean;
    /** 🔴 [2026-09-23] 第 10 口主将队（row -1，前排正中再往前） */
    commander?: boolean;
}
...
interface DecorSprite {
     ...
     * 播完（t ≥ dur）→ 切 rubble 残骸常驻 / 城墙留 D75 残垣。
     * 未设置 = 正常静态装饰。
     */
    collapse?: { t: number; dur: number; rubbleAsset: string | null };
}
/** 装饰层地面贴片（沙滩/水塘/道路/农田等，按 clump 生长的单元格铺 DE 地形贴图） */


type DecorPatch = GroundPatch;
...
export class Scene13WarLayer {
    ...
    /**
     * 每方巡逻航点切换后的驻足停留剩余秒数（见 ROUTE_HOLD_SEC）：>0 时该方走航路的兵站住不动，
     * 搜索残敌；视野内出现敌人（aimAt ①）仍照常锁敌交战，停留只压住「没人可打时走航路」这一路。
     */
    private routeHoldT: [number, number] = [0, 0];
    ...
    private setupPlayerUnits(init: Scene13WarInit, depth: number, midY: number): void {
        ...
        for (const s of this.spawns) {
            ...
            if (setup.control === 'all') this.playerCtlLanes.add(s.lane);
                ...
                else if (s.row <= 0 && !s.commander) this.playerCtlLanes.add(s.lane);
            }
        }
        if (setup.control === 'one') {
            // 探马：同兵种里挑一队
            const pick = this.spawns.find((s) => sameKey(s))
                ?? this.spawns.find((s) => s.f === f && s.row === -1 && !s.commander)
                ?? row0[0];
    ...
    public attach(): void {
        ...
        const onResize = () => {
            ...
            this.initTerrain();        // 尺寸变了 → 按新尺寸重铺（贴图已缓存则立即铺满）
            if (this.decor) {
        ...
        try {
            ...
            if (laneCount[0] !== SIDE_LANES || laneCount[1] !== SIDE_LANES) {
                console.warn(
                    ...
                    + `应恒为 ${SIDE_LANES} 口（主人 2026-09-23 定 10 队：编制 9 + 主将队 1）。`,
                );
            }
    ...
    /**
     * 建筑风格集：**势力专属 → 文化区** 两层。
     * 风格集是整套的（兵营/房屋/塔/墙/门一起换），所以势力级映射只给
     * 「本该用另一套风格集」的势力（安第斯/波斯/东欧/南亚），见 FACTION_BUILDING_STYLE。
     */
    ...
    /**
     * 守方城堡素材：**势力专属 → 文化区 → 风格集默认** 三层。
     *
     * 🔴 [2026-08-26] 只有这样才用得上 DE 那 56 个文明专属城堡 —— 它们在
    ...
    private stepEffects(dt: number): void {
        ...
        // 炮弹/手榴弹/投石落地瞬间 → DE 特效与音效。
        for (const a of this.arrows) {
            if (a.t >= (a.delay ?? 0) + a.dur) {
                const hx = a.x + a.dx * a.len;
                const hy = a.y + a.dy * a.len;
                if (a.proj === 'PROJ_GRENADE') {
                    this.explode('FX_PETARD', hx, hy);
                } else if (a.proj === 'PROJ_BALL' || a.proj === 'PROJ_BOMBARD_BALL') {
                    this.explode('FX_EXPLOSION', hx, hy);
                } else if (a.proj === 'PROJ_MANGONEL' || a.proj === 'PROJ_ROCK') {
                    this.siegeImpact(hx, hy);
                }
            }
        }
    ...
    private slotsOf(region: string, factionId?: string | null, generalId?: string | null): { key: string }[] {
        ...
        return [
            ...
            { key: 'light_infantry' },   // 第 10 口：主将队（兜底同前排兵种）
        ];
    }
    ...
    private initTerrain(): void {
        ...
        this.groundPainter.setTerrain(
            this.environmentPlan?.baseTerrain ?? DEFAULT_TERRAIN_TILE,
            this.canvas.width,
            this.canvas.height,
        );
    }
    ...
    // ── 地面渲染委托给 Scene13GroundPainter（2026-08-24 抽出，与背景图预览工具共用同一份）──
    private groundPainter = new Scene13GroundPainter(() => this.scheduleDecorRepaint());
    private get isoOx(): number { return this.groundPainter.isoOx; }
    private get isoOy(): number { return this.groundPainter.isoOy; }
    private get isoGw(): number { return this.groundPainter.isoGw; }
    private get isoGh(): number { return this.groundPainter.isoGh; }
    ...
    private cityRoadFoundationTile(): string {
        ...
        if (this.defenderCityType === 'pass') return 'rck';        // 险要（关隘）：岩石/硬岩地基
        // 小城 保持文化自适应：中东 rd5 砾石土路 / 罗马·拜占庭 rd1 石板 / 默认 rd2 碎石
    ...
    private applyBuildingsForSide(f: 0 | 1): void {
        ...
        if (this.battleType === 'siege' && f === 1) {
            ...
            const placeGate = (s: { x: number; y: number }, asset: string, dir: 'NE' | 'SE'): void => {
                ...
                for (let step = -3; step <= 3; step++) {
                    ...
                    const extra: DecorSprite = {
                        asset: '', frame: 0, x: s.x + step * 24, y: s.y + step * 12 * dSign, flip: false, layer: 'world', z: 1,
            ...
            const vw = this.canvas?.width ?? 1920;
            // 🔴 [主人明确指示 2026-08-22] 正统 C 型三面包卫宏伟城堡要塞防线：正面垂直主长城向前推进 120px，彻底包裹全部内城建筑
            const wallFrontX = Math.round(wMinX - 380);
    ...
    private applyEnvironmentPlan(): void {
        ...
        if (!plan) return;
        // 设网格 + 高程（同时作废抬升几何与高地光照缓存，下一帧重算）。
        // initTerrain 在本方法之前跑，那次铺地还不知道新高程；这里拿到网格后立刻重铺一次，
        // 保证抬升几何一定对得上（不依赖贴图 onload 的时序）。
    ...
    private repaintDecor(): void {
        ...
        this.groundPainter.paintShading(g, cv.width, cv.height);
    }
    ...
    private ensureType(key: string, side: 0 | 1): void {
        ...
        try {
            ...
            const isDE = _firstUrl.includes('/SUCAI/') || DE_DYN_DIRS.some(dir => _firstUrl.includes(dir));
            b.isDE = isDE;
            if (isDE && !existing) {
    ...
    /** 投石/巨石落地冲击：DE 烟尘碎石飞溅 + 攻城撞击音效（非火药爆炸）。 */
    private siegeImpact(x: number, y: number): void {
        this.spawnFx('FX_TREBUCHET_SMOKE', x, y, 0);
     ...
     *    箭塔：完整 40% / 破损(DAMAGED) 30%（都继续射击）/ 残骸(直接切 RUBBLE) 30%（停火）；
    ...
    private aimAt(m: WarMan): { x: number; y: number } | null {
        ...
        // 【寻敌驻足】航点刚切换的停留期间返回 null（不更新移动目标）→ 走航路的兵站住不动，
        // 停下来搜索残敌。① 视野内已有敌人时上面 search 已接管，这里只压住「没人可打时走航路」这一路。
    ...
    private step(dt: number): void {
        ...
        // 寻敌驻足计时递减：航点刚切换时停留 ROUTE_HOLD_SEC 秒搜索残敌，停够再继续走航路
        for (let f = 0; f < 2; f++) if (this.routeHoldT[f] > 0) this.routeHoldT[f] -= dt;
        this.advanceRoute();
        ...
        for (const b of this.wallGates) { b.atkers = b.atkNext; b.atkNext = 0; }
