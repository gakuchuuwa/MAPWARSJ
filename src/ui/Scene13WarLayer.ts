...
const WAR_TYPES: Record<string, WarType> = {
    ...
    //    病史：耶律大石（非兵力优势 1.75）打十箭（STEPPE 纯骑 6 口三角 5 口突骑）输——纯突骑守军
    //    风筝混编攻方，名将优势兑现不了。实测 kite 60 只改这一件事：突骑打步兵照赢（损 ~75-82%，
    //    「弓骑略强」保留）、被骑兵克照旧、混编攻 vs 纯突骑守 k=1.32 → 攻方 3/3 赢。
    horse_archer:   { name: '突骑兵', cls: 'cav', sz: 1.15, kite: 60, rng: 160, dmg: 15 },
    // ── 远程 5 ──
...
/** 属性组（主人 2026-08-11 三类定稿；弓骑走骑兵组 + 射击；2026-08-12 远程 15/70→28/90 修断边） */
const CLS_STATS = {
    melee:  { hp: 150, dmg: 45, spd: 55, rng: 0 },
    cav:    { hp: 130, dmg: 45, spd: 130, rng: 0 },
    ranged: { hp: 90,  dmg: 28, spd: 50, rng: 160 },
} as const;
...
/**
 * 克制系数 C（2026-08-11 主人采纳 CC 对称方案，一个数调全三角松紧）。
 * 相克表（循环：近战克骑兵 → 骑兵克远程 → 远程克近战）：
 ...
 *
 * ④ **镜像脚本 scratch/*.mts + war_sim.mjs 不含游戏层的两个放大器**（gangMul 围殴加成、
 *    补兵优劣势缩放 LEAD_SCALE）。所以镜像结论**偏保守**：三边相对关系可信，
 *    绝对强度必须实机看。曾有人把镜像里的全歼归因为围殴加成——镜像里根本没有那东西。
 *
 * ⑤ **两病分治**：远程太强 → 动远程数值；整体太一边倒 → 动 GANG_K（全局放大器，三条边同时受影响）。
 *    别混着调，否则分不清是哪个在起作用。
 * ──────────────────────────────────────────────────────────────────────────
 *
 * 🔴 C 的取值是模拟实测定的：1.25/1.5 时「远程克近战」不成立（远程血 70 太脆，
 *    贴身后近战 45÷C 秒杀它，远程那点 ×C 加成在冲过来 1.7 秒里不够打穿 150 血）；
 *    C=1.8 是成立的最小值（8 局换边实测：近战克骑兵 100%、骑兵克远程 100%、远程克近战 100%）。
 *    C=1.8 → 每条边约 1.8 倍差距，符合主人「相克不要太大」。
 *    [2026-08-12] 远程 15/70 → 28/90（修「远程克近战」断边，见 CLS_STATS）：1v1 白嫖 1.4~1.9s 后
 *    肉搏 50.4 DPS 打近战、近战 25 DPS 打 90 血 —— 远程实战胜（剩 ~50 血）。C 不动。
 */
const COUNTER_C = 1.8;
/** 循环克制表：我克谁。melee→cav、cav→ranged、ranged→melee */
const COUNTERS: Record<string, string> = { melee: 'cav', cav: 'ranged', ranged: 'melee' };
 ...
 *    比「分批投入（第一批死光再上第二批）」好：波次之间会空场，看着像刷怪；
...
export class Scene13WarLayer {
    ...
    public start(init: Scene13WarInit): void {
        ...
        try {
            ...
            // 开局总量存档：补兵触发线的优劣势缩放按「剩余占比」算，需要这个分母（见 LEAD_SCALE_MIN）
    ...
    private step(dt: number): void {
        ...
        for (const m of this.men) {
            ...
            if (foe) {
                ...
                // [2026-08-11 主人采纳 CC 对称方案] 循环克制：近战克骑兵 → 骑兵克远程 → 远程克近战。
                ...
                //     射近战按远程克近战 ×C（骑射克步兵），被远程射仍按骑兵吃 ÷C 减伤。
