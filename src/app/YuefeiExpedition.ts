 ...
 *   （13,000 − 0~2,000 随机浮动）→ 每场攻城处于略有优势/略有劣势的胶着区间。
 *   ⚠️ 补员必须在「战后结算」而非「沿途按真实时间涓流」：setInterval 按真实时间、
 *      行军按游戏时间且受倍速影响，短程/5 倍速下涓流严重补不够（曾致第二战败于开封）。
 *   回填上限压到 1.3 万（不是 3 万）：实机守军仅 5,000~10,000（城型初始驻军），
 *      3 万会 6 倍碾压小城且全程 100% 无悬念；压到 1.3 万后小城 ~2.4 倍略优、
 *      大城/关隘 ~1.2 倍险胜，只动军团自身、不碰任何敌方数据。
 *   仿真 400 局（实机初始驻军）：单次直捣黄龙 72%，两次至少一成 92%（主人定的悬念档）；
 *      起兵 2 万 → 前两战（新郑/开封）势如破竹拿下中原门户，之后稳定 1.2 万，越往辽东越吃紧。
 */
...
const ZHONGYI_REFILL_MAX = 13000;
const ZHONGYI_REFILL_JITTER = 2000;
