...
export class LegionPhalanxDrawer {
    ...
    private static async _loadNavalAssets(keys: readonly string[]): Promise<void> {
        ...
        for (const key of keys) {
            ...
            //    所有 /SUCAI/ 素材目录自动加载 _meta.json（内部有 dynMetaCache 缓存，404 会缓存 null），杜绝白名单漏配切碎贴图。
            const _firstUrl: string = (config.MOVE?.[0] ?? config.ATTACK?.[0] ?? config.IDLE?.[0] ?? config.DEATH?.[0] ?? '') as string;
            if (typeof _firstUrl === 'string' && _firstUrl.includes('/SUCAI/')) {
    ...
    public static drawNaval(
        ...
        //    state === 'ATTACK'，航迹被整个丢掉 → buildNavalPath 退化成直线 → 后随船排成一条笔直的队，
        //    旗舰绕椭圆走、整条直线跟着刚体甩，就是主人说的木棍。而 navalFieldPose.trail
        //    （旗舰真实走过的航迹）明明已经算好传进来了。
        //    换回 trailAlignedWithHeading（8-21 为「待命船队沿旧航迹堆叠」写的，此前一直是死代码）：
        //      · 海战绕行 / 行军：航迹切线 = 当前航向 → 同向 → 沿航迹排，后随船跟着弯，保持行军时的蛇形；
