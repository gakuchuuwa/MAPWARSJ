...
export class BattleField implements IOpeningPulseSink {
     ...
     *    等于挨打后才后撤，观感上就是站着对砍。
     *
     *    war_sim 实测（BONUS=1.32，5 种子，**直接改 TYPES 里的 kite**）：
     *      kite 60 → 骑兵克突骑·骑兵余兵 25386 ｜ 步兵打突骑·突骑余兵 21548
     *      kite 63 → 24666 ｜ 23488
     *      kite 66 → 23152 ｜ 24548
     *      kite 70 → 23138 ｜ 26002
     *    骑兵那一侧在 66 触底后就平了（65 是接战距离，越过即生效）；突骑对步兵那一侧则是
     *    一路渐强。取 70：与原型 __war.html 一致，风筝带最宽、观感最明显。
     *    代价：骑兵克突骑的优势 -8.9%（克制方向不变，骑兵仍 5/5 赢，名将优势照常兑现）；
     *    突骑对步兵 +20.7%。
     *
     *    ⚠️ 别用 `KITE=xx` 环境变量测这件事：war_sim 里 `kiteV = KITE_OVERRIDE ?? T.kite`，
     *    该变量会把风筝**加到交战双方所有兵种头上**（连近战骑兵、步兵都会后撤），
     *    测出来的差异比真实情况大三倍（2026-08-19 first pass 就是这么误报成 -26% 的）。
     *    要测就复制 war_sim.mjs 改里面的 kite 值。
     *
