/**
 * 西域文化区远征精锐军团名（GAME_DIRECTION 远征细则 2026-06-11）
 *
 * 收录红线与日朝东北草原一致：
 * - 番号至少 3 个汉字
 * - 同势力只挂一个番号（比知名度分流）
 * - 不收热兵器专名（§14 #9 叶尔羌火枪兵等）
 * - 依据 史料/古代精锐部队.md §14 #1–8；#10 喀喇契丹无合格势力故不收
 */
export const WESTERN_EXPEDITION_ELITE_LEGIONS: Readonly<Record<string, { name: string; tier: 0 | 1 | 2 | 3 }>> = {
  qiuci: { name: '龟兹重甲兵', tier: 3 },   // 伊逻卢·龟兹国冶铁重装（§14 #1）
  yutian: { name: '于阗尉迟军', tier: 3 },  // 于阗·尉迟王族精锐（§14 #2）
  kala: { name: '喀喇汗铁骑', tier: 1 },    // 奥什·喀喇汗国铁骑（§14 #3）
  yiduhu: { name: '高昌铁骑', tier: 1 },    // 高昌·麴氏高昌具装骑（§14 #4）
  shule: { name: '盘橐卫', tier: 3 },       // 盘橐城·疏勒强弩守军（§14 #5；班超大本营）
  yanqi: { name: '焉耆龙骑兵', tier: 3 },   // 员渠城·焉耆龙骑（§14 #6）
  wusun: { name: '昆莫亲卫', tier: 1 }, // 赤谷城·乌孙王帐亲卫（§14 #7）
  chagatai: { name: '蒙兀儿铁骑', tier: 1 }, // 别失八里·东察合台蒙兀儿铁骑（§14 #8）
  dayuan: { name: '汗血天马骑', tier: 3 },   // 贵山城·史记「汗血天马子」；贵山守战四十余日抗汉（大宛列传）
  shache: { name: '莎车左右骑', tier: 3 },   // 渠莎·汉书莎车「左右骑君」；后汉莎车王贤数万人征服诸国
  anxi: { name: '大唐安西军', tier: 3 },     // 拨换城·高仙芝/郭昕安西都护府精锐（§1 #54）
  // §14 #9 叶尔羌火枪兵（火绳枪）→ 不收
  loulan: { name: '楼兰戍', tier: 3 },      // 扜泥城·汉晋楼兰戍卒（旗=楼兰·§12.1.1）
  // §14 #10 喀喇契丹重骑兵 → 项目无西辽/喀喇契丹独立势力，宁缺毋滥
  // ── 2026-06-16 新增：西域大区平衡补全（5支，精锐随据点，全图无六字番号） ──
  quli: { name: '轮台戍卒', tier: 2 },
  dzungar: { name: '塔城卫队', tier: 3 },
  yarkand: { name: '英吉沙骑兵', tier: 3 },
  tuerhute: { name: '鹰娑川铁骑', tier: 1 },
  gaochang: { name: '赤亭关守军', tier: 3 },
};
