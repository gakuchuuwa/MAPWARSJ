...
const _FIXED: Record<string, string> = {
    ...
    'suogediyana': '#6E5B45', // 索格狄亚那·岩堡绝壁（灰褐；与粟特的红袍金带分色）
    ...
    'funan': '#C26E28',
    'lancang': '#E09A18',
    'liaozu': '#3F9F2F', // 万象·寮族军团（暗绿，压低战象大面积玩家色亮度）
    'ahaomu': '#984028',
    'elunchunzu': '#5A4838',
    'wazu': '#2D3033',
    'tajikezu': '#854030',
    ...
    'mamuluke': '#C99700', // 马穆鲁克（苏丹旗黄底狮徽·金黄）
    ...
    'yiaoniya': '#2F6F5E', // 伊奥尼亚·米利都（爱琴海深青，与以弗所的爱奥尼亚青分色）
    ...
    'sulu': '#1B6E8A', // 苏禄·霍洛（苏禄海珍珠蓝）
    'xishudongyin': '#8A1E32', // 西属东印·马尼拉（西属红·王城）
    ...
    'puxiangyindu': '#7A5A2E', // 葡属印度·果阿（印尼-葡萄牙象牙金，果阿城与香料贸易/火枪兵）
    'xiyindu': '#14506E', // 西印度·圣多明各（加勒比深青蓝，西印度总督府/奥萨马堡）
    'jialebi': '#1E5B4F', // 加勒比·卡塔赫纳（加勒比海蓝绿，南美最坚固要塞/海防石墙）
    ...
    'wangdaer': '#4A5A63', // 汪达尔·盖萨里克（海岩风暴灰）——原 #5C6B73 与霍州(huo)撞色，整个模块 throw 导致全站白屏，2026-09-06 让路加深一档
    'guanche': '#8A6B4A', // 关切·特尔德（加那利羊皮褐；关切人属柏柏尔系）

    // ── 2026-08-30 补齐全部正规势力固定色：有旗帜/纹章从其主色，无统一国色则取史地物质文化色 ──
    'yelusalengwg': '#C8A23A', // 耶路撒冷王国·阿卡（金色耶路撒冷十字）
    'aztec': '#287F8B', // 阿兹特克·特诺奇提特兰（绿松石与翠羽贵重色）
    'baiyiya': '#3D6FA3', // 巴伊亚·萨尔瓦多（葡属巴西大西洋海蓝）
    'tupinijin': '#A64B32', // 图皮尼金·圣维森特（乌鲁库胭脂木赭红）
    'tuotuonake': '#D0A84B', // 托托纳克·韦拉克鲁斯（香草与玉米金）
    'taino': '#3D91A8', // 泰诺·哈瓦那（加勒比海青）
    'inca': '#9E2636', // 印加·库斯科（萨帕印卡红色王冠流苏）
    'maya': '#2F7290', // 玛雅·蒂卡尔（玛雅蓝）
    'mapuche': '#355F91', // 马普切·图卡佩尔（南部山海深蓝）
    'muisca': '#C49A2E', // 穆伊斯卡·巴卡塔（穆伊斯卡黄金工艺）
    'tupi': '#9C3F2D', // 图皮·瓜纳巴拉（乌鲁库胭脂木红）
    'xibanya': '#B52B34', // 西班牙·利马（勃艮第十字绛红）
    'manding': '#B8862F', // 曼丁哥·廷巴克图（马里黄金贸易）
    'ethiopia': '#397545', // 埃塞俄比亚·阿克苏姆（所罗门王朝绿）
    'malacca': '#8E2638', // 满剌加·马六甲（马来苏丹王权绛红）
    'medang': '#A8742C', // 马打蓝·卡拉桑（中爪哇佛寺赭金）
    'kushi': '#5A3D2E', // 库施·麦罗埃（努比亚黑土与金饰）
    'aimala': '#6F4A82', // 艾马拉·蒂亚瓦纳科（安第斯织物紫）
    'aolisha': '#C66F28', // 奥里萨·普里（贾格纳特祭仪藏红）
    'kanata': '#B85A24', // 卡纳塔·亨比（毗奢耶那伽罗藏红赭）
    'adile': '#2F6B58', // 阿迪勒·比贾布尔（德干苏丹国青绿）
    'foluolida': '#8F3038', // 佛罗里达·圣奥古斯丁（西班牙勃艮第十字暗红）
    'fujisi': '#4F7651', // 福基斯·德尔斐（阿波罗月桂绿）
    'yilisi': '#687A3A', // 埃利斯·奥林匹亚（奥林匹亚野橄榄绿）
    'nuowei': '#A92F36', // 挪威·博尔贡（挪威王国旗红）
    'keernuwaye': '#292929', // 康沃尔·坎佩尔（圣皮兰黑地白十字）
    'aodesuosi': '#356B82', // 奥德索斯·瓦尔纳（黑海港深蓝）
    'disidelusi': '#8A6046', // 蒂斯德鲁斯·杰姆（罗马圆形剧场砂岩褐）
    'yisatisi': '#B85C2D', // 伊萨提斯·亚兹德（琐罗亚斯德圣火橙）
    'wuer': '#315C8A', // 乌尔·乌尔城（王陵青金石蓝）
    'pidisha': '#B77A38', // 毗底沙·桑奇（桑奇佛塔砂岩赭）
    'jiaye': '#C58A2F', // 伽耶·菩提伽耶（佛教袈裟赭黄）
    ...
    'qimu': '#C09640', // 奇穆·昌昌（奇穆海岸土坯城金）
    'talasike': '#B06040', // 塔拉斯科·钦聪灿（普雷佩查铜器工艺铜）
    'tailuona': '#2E6E44', // 泰罗纳·特尤纳（失落之城雨林绿）
    'teweierqie': '#5A6E82', // 特维尔切·圣胡利安港（巴塔哥尼亚寒原灰蓝）
    'xinnidelan': '#D86428', // 新尼德兰·新阿姆斯特丹（荷兰奥兰治橙）
    'xinfalanxi': '#2A4A88', // 新法兰西·魁北克（法兰西王室蓝）
    'xinxibanya': '#B53828', // 新西班牙·阿卡普尔科（西班牙勃艮第十字绛红）
    'dunhe': '#2E5A6E', // 顿河·维申斯卡亚（顿河哥萨克河蓝）
    'yamaxun': '#4A2E5E', // 亚马逊·忒弥斯基拉（黑海女战士暗紫）
    'manzheboyi': '#9C3240', // 满者伯夷·特罗武兰（满者伯夷红白旗绯红）
    'zhelouqi': '#C07A1E', // 遮娄其·阿杰梅尔（拉其普特藏红琥珀）
    'suosuo': '#4A3626', // 索索·库姆比萨利赫（坎特铁匠部族乌褐）
    'antiaokegongguo': '#982026', // 安条克公国·拉塔基亚（诺曼十字军暗红）
    'aoerlianggongguo': '#2A5E9E', // 奥尔良公国·兰斯（法兰西王室蓝）
    'jinquehua': '#A8322A', // 金雀花·卡那封（金雀花王朝红）
    'apuliya': '#8A5A2E', // 阿普利亚·梅尔菲（诺曼南意大利土褐）
    'saimien': '#3E7A44', // 塞米恩·塞米恩（塞米恩山地绿）
    'aosimanbeiyiguo': '#9E2620', // 奥斯曼贝伊国·瑟于特（奥斯曼红）
    'aiduoniya': '#8A2038', // 埃多尼亚·安菲波利斯（斯特里蒙河色雷斯绯红）
    'qiesuonisuosi': '#1E5E8C', // 切索尼索斯·羊河（爱琴海海军蓝）
    'mixiya': '#7E2A8C', // 密西亚·格拉尼库斯（小亚密西亚紫）
    ...
    'telakaigongguo': '#3E7A5E', // 特拉凯公国·卡乌纳斯（立陶宛森林绿）
    'duobuluojia': '#6E8A4A', // 多布罗加·德鲁斯塔尔（多瑙河绿）
    'aertai': '#5A6E7A', // 阿尔泰·也儿的石（阿尔泰山灰蓝）
    'kumanni': '#4A7A8C', // 库曼尼亚·卡尔卡河（钦察草原钢蓝）
    'xikuman': '#7A6E4A', // 西库曼·密尔科夫（喀尔巴阡褐）
    ...
    // 🔴 [2026-09-19 主人指出审计警告「缺 HistoricalFactionColors 势力色」] 补齐 8 个新势力的色。
    //    取色按本文件头两条：**有尚色依据用尚色**；无依据则**按史地特征取色**（水师海蓝 / 山地岩色 / 河畔沙黄…）。
    //    ⚠️ 每一条都经脚本实测：与全表 1000+ 色 RGB 欧氏距离 ≥ 28（`RESERVED_COLOR_MIN_RGB_DISTANCE`），无撞色、无近色。
    'genji': '#989791', // 源氏·镰仓（白旗之银灰调；**纯白位已被全表 29 条浅色占满**，此色是实测最近的可用浅灰）
    'genji_iyo': '#108EE8', // 源氏·伊予·松山（忽那水军·海蓝；本文件原则「水师用海蓝」）
    'aswaka': '#799F88', // 阿斯瓦卡·马萨加（斯瓦特山谷·灰绿）
    'aornos': '#7B6A52', // 奥诺斯岩·崖壁灰褐（与索格狄亚那岩的深灰褐分色）
    'julushi': '#8A6F4A', // 居鲁士城·粟特边塞土黄（与粟特红袍、索格狄亚那灰褐分色）
    ...
    'lulisitan': '#28CA28', // 卢里斯坦·伊拉姆（山前灰褐）
    'golpayegan_diqu': '#28D05E', // 古尔帕耶甘地区（山间谷地）
    'meisuobudamiya': '#7F6A3F', // 美索不达米亚·尼西比斯（两河平原·深土）      // 阿蒙·锡瓦绿洲（沙漠神庙·赭金） // 卡帕多细亚·提亚纳（安纳托利亚高原·土黄。无尚色记载 → 按史地特征取色） // 达萨雷提·佩利昂（伊利里亚山林·深松绿。无尚色记载 → 按史地特征取色）
};
