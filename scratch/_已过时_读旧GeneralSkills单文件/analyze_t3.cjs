...
const t3Generals = new Set();
const t3Names = {};
for (const line of lines.slice(header + 2)) {
    if (!line.trim().startsWith('|')) continue;
    const parts = line.split('|').map(s => s.trim());
    if (parts.length >= 10 && parts[10] === 'T3') {
        const genName = parts[5];
        const genId = nameToGenId[genName];
        if (genId) {
            t3Generals.add(genId);
            t3Names[genId] = genName;
        }
    }
}
...
let out = '# T3 武将专属神装越权查处报告\n\n';
let mismatches = 0;

for (const genId of Array.from(t3Generals)) {
    ...
    const checkSkill = (skillId, slot) => {
        ...
        if (EXCLUSIVE_MAPPING[skillId]) {
            ...
            if (!desc.includes(t3Names[genId]) && !desc.includes(genId)) {
                isMismatch = true;
            }
        }
        
        if (isMismatch) {
            out += `- **${t3Names[genId]}** (T3) 错误持有了: 【${sName}】 (${slot}) -> 描述: ${desc || '无'}\n`;
            mismatches++;
        }
    };
...
out += `\n总计发现 ${mismatches} 处 T3 级别越权配置。\n`;
fs.writeFileSync('C:/Users/GAKU/.gemini/antigravity/brain/28d1eb5e-ad88-45e0-806c-3cb19e09c128/t3_mismatches_report.md', out, 'utf-8');
console.log('T3 扫描完成，发现疑点:', mismatches);
