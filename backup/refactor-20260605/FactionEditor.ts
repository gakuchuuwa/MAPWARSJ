 ...
 *   3. src/app/GameApp.ts                    → STARTING_CAPITALS
...
export class FactionEditor {
    ...
    private async previewBatch(): Promise<void> {
        ...
        for (let i = 0; i < entries.length; i++) {
            ...
            // pass:    关/口/塞/陉/径/隘 → 8000 兵
            // medium:  府/京/都           → 10000 兵
            // small:   默认 (城/堡/寨/卫/...) → 5000 兵
            let typeBadge = '';
            const n = entry.cityName;
            const passKey = ['关','口','塞','陉','径','隘'].find(s => n.endsWith(s));
            const medKey  = ['府','京','都'].find(s => n.endsWith(s));
            if (passKey) {
                typeBadge = ` <span style="color:#FFB74D;font-size:9px;background:#5d4037;padding:0 3px;border-radius:2px;" title="关隘 pass (8000 兵)">${passKey}</span>`;
            } else if (medKey) {
