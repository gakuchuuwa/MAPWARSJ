...
# scale 钳制：只缩不放——上限 1.0，A 永远不放大任何头。
# ⚠ 血训一：曾按"头一样大"硬算 → 小头图被放大到 1.5+（如 WEST_ASIA_50 头高 0.21 → scale 1.55）。
# ⚠ 血训二：上限收到 1.21 仍被嫌"头变大"；收到 1.10，主人调好的图点 A 还是会被抬到 1.10，
#    第三次被骂"你把头部搞那么大干什么"。
# ⚠ 定论（2026-08-03 主人拍板）：主人三轮反馈全部是"头变大"，从没嫌过头小；
#    他自己 400 条手调里"需要放大 1.1~1.2 的图"中位也只给 1.04——从不放大。
#    所以 A 的语义定死为【只缩不放】：大头收到 TARGET_HEAD，小头/正常头保持原样。
#    代价：小头图不会被拉平到目标（本来就是主人自己的取舍），两图并排小头略小属预期。
SCALE_MIN, SCALE_MAX = 0.85, 1.0
