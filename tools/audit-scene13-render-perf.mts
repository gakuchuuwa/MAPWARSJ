...
if (!body) bad('找不到 renderDynamicWater（结构变了，先看代码再改这个脚本）');
    ...
    // ③ 有效水域必须按包围盒裁切；没有几何形状的 patch 不参与绘制。
    if (/const bb = this\.waterBBoxOf\(p\);\s*if \(!bb\) continue;/.test(body)
        && /if \(bw <= 0 \|\| bh <= 0\) continue;/.test(body)) {
        ok('按水域几何计算包围盒，空区域跳过');
    } else bad('缺少有效水域包围盒或空区域检查');

    // ④ maskCv 已是局部尺寸：drawImage(maskCv, 0, 0) 只模糊水域，不是整张战场。
    if (/maskCv\.width = bw;/.test(body) && /maskCv\.height = bh;/.test(body)
        && /softMask\.width = bw;/.test(body) && /softMask\.height = bh;/.test(body)) {
        ok('遮罩与羽化画布均限制在水域局部尺寸');
    } else bad('水面遮罩或羽化画布未限制在局部尺寸');

    if (/this\.composedWaterPattern\(img, pat, t\)/.test(body)
        && /canvas\.width = img\.naturalWidth;/.test(SRC)
        && /canvas\.height = img\.naturalHeight;/.test(SRC)
        && /if \(tile\.at === t\) return tile\.pattern;/.test(SRC)) {
        ok('双层水纹与微光在纹理尺寸内合成，同帧同素材复用');
    } else bad('纹理合成未限制尺寸或缺少同帧复用');
