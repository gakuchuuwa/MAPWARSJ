...
# 🔴🔴 `t_cold` = **bio6 = Min Temperature of Coldest Month（最冷月的最低温）**，
#    **不是最冷月均温**！我一直当均温用，注释里也写错了（"莫斯科最冷月 -10.4" 其实是 bio6，
#    莫斯科一月均温是 -6.5）。于是阈值偏暖：-3 相当于均温 +1~2°C，
#    把洛阳（bio6 -5，冬天大部分时间地面是土）判成了雪原。
#    按「什么最多」重定：真雪区 = 地面大部分冬天被雪盖 → bio6 < -7。
WINTER_SNOW_TC = -7.0
# 🔴 [2026-08-24 主人提：「冬天是不是应该给其他背景添加一点雪地地基作为二层？」]
#    -3 ~ +2°C 这一档：**会下雪但存不住**——江南北部、华北南部、地中海北岸、中欧、
#    日本西南、朝鲜南部。这些地方冬季地面不是纯土也不是雪原，是**零星残雪**。
#    此前只有「有雪／无雪」两态，这一带被判成完全无雪，冬天和夏天一个样，不真实。
#    标志位 4：底图仍用非冬季那张，但引擎会在上面铺少量雪斑（见 §5.6.17）。
SPARSE_SNOW_TC = 0.5
# 深雪只给终年冰冻的极地与高山。
...
def main():
    ...
    # 4 = 零星残雪：会下雪但存不住，底图不换、只在上面铺少量雪斑
    wflag[land & (t_cold >= WINTER_SNOW_TC) & (t_cold < SPARSE_SNOW_TC)] = 4
    ...
    (OUT / "world-base.json").write_text(json.dumps({
        "note": "任意经纬度 → 底图查找图。R=攻城战(非冬) G=野战(非冬) B=冬季状态(0无雪/1雪地/2深雪/3雪林/4零星残雪)。"
