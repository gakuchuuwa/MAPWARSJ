 ...
 *
 * 判据：每张 __多余__ 都必须存在一份【非 __多余__】的同 SHA-256 副本兜底
 *       （被引用正本 / __闲置__ / chongfu 对照夹都算安全副本）。
...
for (const s of surplus) {
    ...
    const safeTwin = peers.some((o) => !SURPLUS_RE.test(o.file) && sha(o) === sha(s));
    if (!safeTwin) orphans.push(s);
}

console.log(`[多余图删前自查] 全库 ${files.length} 张 | __多余__ ${surplus.length} 张`);
if (orphans.length === 0) {
    console.log(`✅ 全部 ${surplus.length} 张多余图都有安全副本兜底 —— 可以放心全部删除，不会丢图`);
} else {
    console.log(`❌ 有 ${orphans.length} 张是库内最后一份，删了就绝版（名字上的「多余」标签已过期）：`);
    for (const o of orphans) console.log(`   ${o.folder}/${o.file}`);
    console.log('\n   先跑 npm run portrait:dup-rename 让它们自动改回 __闲置__，再回来复查。');
}
process.exit(0);
