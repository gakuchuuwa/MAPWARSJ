...
const idle = [...assets].filter((a) => !used.has(a)).sort();
console.log(`  ${assets.size} 座 → 安置 ${used.size} 座（${Math.round(used.size / assets.size * 100)}%）`);
if (idle.length) bad(`仍闲置：${idle.join(', ')}`);
else ok('零闲置');
