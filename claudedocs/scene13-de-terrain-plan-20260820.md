...
> 🔴 **2026-08-20 顺序已调整**：原 P1（biome 判定）与原 P2（植被 DE 化）**对调**。
> 原因：P0 落地后地面成了 DE 像素草皮，树却还是三国群英传的橙红树丛，
> **半 DE 半三国比全三国还难看**。先统一画风，再谈地形细分。

| 期 | 内容 | 状态 / 验收 |
|---|---|---|
| **P0** | 转 3 张草地（`gr3 gr6 gr8`）＋铺地代码，硬编码温带森林 | ✅ 已完成。铺法二次修正：原尺寸 512 铺（原先压成 256 失真）＋每块镜像/旋转打散重复（原先 256px 一个周期，肉眼可见棋盘条带） |
| **P1** | 转 DE 植被（**只提温带组 20 个**），**删三国树/湖**，按季节选树种 | 工单见 `ds-worklist-p1-vegetation-20260820.md`。验收：画面里找不到三国树 |
| **P2** | 转全部 83 张地形；L1+L2 判定（卫星色采样 + 海拔）；biome→地形映射落地；补提各 biome 树种 | 换几个不同地区开战，地表明显不同 |
| **P3** | L3 点缀（海滩/河岸/农田/道路/贴花）＋过渡混合 | 细节到位 |

**每期结束都要实机截图交主人验收，验收过了才进下一期。**
...
**已拍板**
1. P0 biome = 温带森林（中原/江南战场最多）。✅ 已落地
2. 植被提取范围 = **只提映射表用到的**，且**按期分批**：P1 只提温带组 20 个，其余 biome 的树等 P2 接上判定再提。

**待拍板**
3. **云怎么办**：AoE2 没有云素材（游戏无天气层）。严格「全 DE」= 去掉云；
   但云是主人 08-12 专门加的动感元素，且半透明白色积云风格中性、混搭违和感最低。
   → (a) 去掉 / (b) 作为唯一例外保留三国云。**在答复前云的代码一行不动。**
