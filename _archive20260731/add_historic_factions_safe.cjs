...
const mappings = [
    { name: '瓦剌', id: 'wala', city: '博尔巴任', lat: 49.664324, lng: 95.767822 },
    { name: '兀良哈', id: 'wuliangha', city: '赛音山达', lat: 44.8751, lng: 110.1544 },
    { name: '丁零', id: 'dingling', city: '燕然山', lat: 46.276728, lng: 102.801819 },
    { name: '尼夫赫', id: 'nifuhe', city: '盆奴里', lat: 47.650566, lng: 130.957031 }
];

// 1. cities_v2.ts
let lines = fs.readFileSync('src/data/cities_v2.ts', 'utf-8').split('\n');
for (let i = 0; i < lines.length; i++) {
    for (const m of mappings) {
        if (lines[i].includes(`name: '${m.city}'`) && lines[i].includes("factionId: 'panjun'")) {
            lines[i] = lines[i].replace("factionId: 'panjun'", `factionId: '${m.id}'`);
        }
    }
}
...
console.log('Successfully updated all 5 files line-by-line safely!');
