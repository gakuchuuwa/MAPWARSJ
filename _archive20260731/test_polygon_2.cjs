...
const NORTH_POLYGON = [
    ...
    {lat: 41.5, lng: 119.0}, // 赤峰附近，长城折角
    {lat: 42.5, lng: 124.2}, // 开原/铁岭略北
    {lat: 39.8, lng: 124.5}, // 鸭绿江口
    {lat: 39.8, lng: 130.0}, // 远海
    {lat: 37.8, lng: 130.0}, // 远海
...
function getRegion(lat, lng) {
    ...
    // NORTHEAST bounds usually > 40 & > 120, but we want NORTH_POLYGON to take precedence
    ...
    if (lat > 40.0 && lng > 120.0) return 'NORTHEAST'; // Northeast fallback for regions outside the NORTH polygon
    ...
    // Fallbacks just in case a boundary point fails polygon test
...
const testCities = [
    '襄平', '皮岛', '平壤', '洛阳', '长安', '天水', '武威', '成都', '延安', '安阳', '大同', '山海关', '乌骨城', '徒河', '赫图阿拉'
];

cities.filter(c => testCities.includes(c.name)).forEach(c => {
    console.log(`${c.name}: ${getRegion(c.lat, c.lng)} (lat: ${c.lat}, lng: ${c.lng})`);
});
