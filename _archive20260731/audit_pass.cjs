...
for(let line of lines) {
    if(line.includes('{ id: ') || line.includes('{id:')) {
        if(currentCityStr.includes("'pass'") || currentCityStr.includes('"pass"')) {
            passes.push(currentCityStr);
        }
        currentCityStr = line;
    } else {
        currentCityStr += '\n' + line;
...
const parsedCities = passes.map(str => {
    return {
...
flagsCode.split('\n').forEach(line => {
    ...
    // This regex carefully matches the key, optionally wrapped in quotes, and the value in quotes
...
const report = {
    totalPages: parsedCities.length,
    nameConflicts: [],
    invalidFlags: [],
    missingFlags: [],
    allData: []
...
fs.writeFileSync('pass_audit.json', JSON.stringify(report, null, 2));
