...
const steppeCities = cities.filter(c => getCityRegion(c) === "STEPPE");

const pairs = [];
for(let i=0; i<steppeCities.length; i++) {
    for(let j=i+1; j<steppeCities.length; j++) {
        const c1 = steppeCities[i];
        const c2 = steppeCities[j];
        const d = haversineKm(c1.latitude, c1.longitude, c2.latitude, c2.longitude);
        pairs.push({
            c1: c1.name,
            id1: c1.id,
            c2: c2.name,
...
console.log("Top 15 closest pairs in STEPPE region:");
for(let i=0; i<15; i++) {
    console.log(`${pairs[i].c1} (${pairs[i].id1}) <--> ${pairs[i].c2} (${pairs[i].id2}): ${pairs[i].distance.toFixed(2)} km`);
}
